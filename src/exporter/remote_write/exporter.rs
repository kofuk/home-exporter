    ...
    async fn real_run(self) -> Result<(), ()> {
        ...
        let mut client = HttpClient::new_with_tls(
            ...
            TlsConfig::new(
                ...
                reqwless::client::TlsVerify::Certificate {
                    ca: include_bytes!("../../../ca.der"),
                    cert: None,
                    key: None,
                },
            ),
        ...
        info!("Sending request to https://example.com");
        let response = request.send(&mut rx_buffer).await.map_err(|_err| {
            error!("Failed to send request");
            ()
        })?;

        let StatusCode(status_num) = response.status;
        info!("Received response with status: {:?}", status_num);
